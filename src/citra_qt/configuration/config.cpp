...
void Config::ReadValues() {
    ...
    if (icon_size < 0 || icon_size > 2) {
        LOG_ERROR(Config, "Invalid value for game_list_icon_size: {}", icon_size);
        icon_size = 2;
    }
    UISettings::values.game_list_icon_size = static_cast<UISettings::GameListIconSize>(icon_size);

    int row_1 = ReadSetting("row1", 2).toInt();
    if (row_1 < 0 || row_1 > 3) {
        LOG_ERROR(Config, "Invalid value for game_list_row_1: {}", row_1);
        row_1 = 2;
    }
    UISettings::values.game_list_row_1 = static_cast<UISettings::GameListText>(row_1);

    int row_2 = ReadSetting("row2", 0).toInt();
    if (row_2 < -1 || row_2 > 3) {
        LOG_ERROR(Config, "Invalid value for game_list_row_2: {}", row_2);
        row_2 = 0;
    }
    UISettings::values.game_list_row_1 = static_cast<UISettings::GameListText>(row_2);
