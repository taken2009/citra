...
#ifdef ENABLE_WEB_SERVICE
    return WebService::VerifyLogin(username, token, Settings::values.web_services_url + "/profile",
                                   func);
#else
...
TelemetrySession::TelemetrySession() {
    ...
    if (Settings::values.enable_telemetry) {
        backend = std::make_unique<WebService::TelemetryJson>(
            Settings::values.web_services_url + "/telemetry", Settings::values.citra_username,
